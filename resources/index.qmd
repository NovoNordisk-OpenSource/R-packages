...
resources <- here::here("_resources.yml") |>
  yaml::read_yaml() |>
...
resources <- split(resources, resources$type) |>
...
for (i in seq_along(resources)) {
  cat("## ", stringr::str_to_title(names(resources)[[i]]), "\n")

  for (j in seq_along(resources[[i]])) {
    cat("### ", stringr::str_to_title(names(resources[[i]])[[j]]), "\n")
